...
char* path;
// initial permutation - used before 1st round.
int IP[] = 
{
...
// Expansion used in F (Feistel function)
int E[] = 
{
      32,  1,  2,  3,  4,  5,
       4,  5,  6,  7,  8,  9,
...
// Permutation used in F (Feistel function)
int P[] = 
{
      16,  7, 20, 21,
      29, 12, 28, 17,
...
// Final Permutation - used after 16th round
int FP[] = 
{
...
// 8 substitution boxes used in F (Feistel function)
int S1[4][16] = 
{
...
// PC1 and PC2 are used in key schedule.
int PC1[] = 
{
      57, 49, 41, 33, 25, 17,  9,
       1, 58, 50, 42, 34, 26, 18,
...
// SHIFTS used in key schedule for generating 16 sub keys of size 48 bits.
int SHIFTS[] = { 1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1 };
 
FILE* out;

// used to hold left and right parts respectively of input in each round
int LEFT[17][32], RIGHT[17][32];
// used to hold input text of 64 bits.
int IPtext[64];
// used to hold expansion text in Fiestal function
int EXPtext[48];
// used to hold xor text after applying xor operation.
int XORtext[48];
// used to hold 8 6 bit values used in feistel function.
int X[8][6];
int X2[32];
// contains output of feistel function after permutaion step.
int R[32];
// used to hold effective key of size 56 bits
int key56bit[56];
// used to hold 16 sub keys
int key48bit[17][48];
// contains output of 16th round.
int CIPHER[64];
// contains encrypted text of input (64 bits).
int ENCRYPTED[64];

// used in F (Feistel function)
...
// used before round 1 of the algorithm
...
// final permutation applied after 16th round.
...
{
    FILE* inp = fopen(concatenate(path,"input.txt"), "rb");
...
{
    out = fopen(concatenate(path,"cipher.txt"), "ab+");
    for (int i = 0; i < 64; i++)
...
int bittochar()
{
    out = fopen(concatenate(path,"output.txt"), "ab+");
...
{
    FILE* in = fopen(concatenate(path,"cipher.txt"), "rb");
    long int plain[n * 64];
    int i = -1;
    char ch;
...
void create16Keys()
{
    FILE* pt = fopen(concatenate(path,"key.txt"), "rb");
    unsigned int key[64];
    int i = 0, ch;
...
{
    if(argc < 2){
        printf("less than number of required arguments\n");
        printf("command should be:\n");
        printf("./des path_test_folder\n");
        return -1;
    }
    path = argv[1];
    // destroy contents of these files (from previous runs, if any)
    out = fopen(concatenate(path,"output.txt"), "wb+");
    fclose(out);
    out = fopen("decrypted.txt", "wb+");
    fclose(out);
    out = fopen(concatenate(path,"cipher.txt"), "wb+");
    fclose(out);
